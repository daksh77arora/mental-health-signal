(() => {
  ...
  const segButtons = Array.from(segGroup.querySelectorAll(".seg-btn"));

  function selectStressButton(btn) {
  ...
  segButtons.forEach((btn) => {
    btn.addEventListener("click", () => selectStressButton(btn));
  });

  segGroup.addEventListener("keydown", (e) => {
    const currentIndex = segButtons.indexOf(document.activeElement);
    if (currentIndex === -1) return;
    let nextIndex = null;
    if (e.key === "ArrowRight" || e.key === "ArrowDown") nextIndex = (currentIndex + 1) % segButtons.length;
    if (e.key === "ArrowLeft" || e.key === "ArrowUp") nextIndex = (currentIndex - 1 + segButtons.length) % segButtons.length;
    if (e.key === "Home") nextIndex = 0;
    if (e.key === "End") nextIndex = segButtons.length - 1;
    if (nextIndex !== null) {
      e.preventDefault();
      segButtons[nextIndex].focus();
      selectStressButton(segButtons[nextIndex]);
    }
  });
  ...
  function setFieldError(input, message) {
    ...
    if (input === stressHiddenInput) segGroup.setAttribute("aria-invalid", "true");
  }
  ...
  function clearFieldError(input) {
    ...
    if (input === stressHiddenInput) segGroup.removeAttribute("aria-invalid");
  }
  ...
  function clearAllErrors() {
    ...
    segGroup.removeAttribute("aria-invalid");
  }
